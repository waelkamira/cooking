...
export default function Categories() {
  ...
  return (
    <motion.div
      ...
      className="w-full py-12 px-6 rounded-3xl relative overflow-hidden"
    >
      {/* Enhanced Background Decoration */}
      <div className="absolute inset-0 pointer-events-none" aria-hidden="true">
        <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-br from-orange-400/50 to-pink-300/5 opacity-20 blur-3xl" />
        <div className="absolute bottom-0 right-0 w-1/2 h-1/2 opacity-20 blur-3xl" />
      </div>

      {/* Main Content */}
      <div className="relative z-10 max-w-7xl mx-auto">
        <h2 className="text-3xl md:text-4xl font-bold text-black text-center mb-8 tracking-tight">
        ...
        >
          {categories.map((category, index) => (
            <motion.div
              key={index}
              whileTap={{ scale: 0.95 }}
              className="keen-slider__slide flex flex-col items-center rounded-xl overflow-hidden cursor-pointer"
