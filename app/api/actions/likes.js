import { supabase } from '../../../lib/supabaseClient';
import { invalidateCacheByPrefix } from '../../../lib/cacheUtils';

export async function handleLikesAction(email, mealId, newActionValue) {
  try {
    // التحقق من وجود سجل مشابه
    const { data: existingAction, error: existingActionError } = await supabase
      .from('Action')
      .select('*')
      .eq('userEmail', email)
      .eq('mealId', mealId)
      .single();

    if (existingActionError && existingActionError.code !== 'PGRST116') {
      throw new Error('Internal Server Error');
    }

    let message;

    // تحديث أو إنشاء السجل بناءً على القيمة الجديدة
    if (existingAction) {
      if (newActionValue === 0) {
        // حذف السجل إذا كانت القيمة الجديدة 0
        const { error } = await supabase
          .from('Action')
          .update({ likes: newActionValue })
          .eq('userEmail', email)
          .eq('mealId', mealId);

        if (error) {
          throw error;
        }
        message = 'تمت إزالة الإعجاب من هذه الوصفة بنجاح';
      } else {
        // تحديث السجل إذا كانت القيمة الجديدة 1
        const { error } = await supabase
          .from('Action')
          .update({ likes: newActionValue })
          .eq('userEmail', email)
          .eq('mealId', mealId);

        if (error) {
          throw error;
        }
        message = 'تمت إضافة الإعجاب لهذه الوصفة بنجاح';
      }
    } else {
      // إنشاء السجل إذا لم يكن موجوداً
      const { error } = await supabase
        .from('Action')
        .insert({ userEmail: email, mealId, likes: newActionValue });

      if (error) {
        throw error;
      }
      message = 'تمت إضافة الإعجاب لهذه الوصفة بنجاح';
    }

    // تحديث التخزين المؤقت
    invalidateCacheByPrefix('actions_');

    return { message, newActionValue };
  } catch (error) {
    console.error('Error handling likes action:', error);
    return { error: 'Internal Server Error' };
  }
}
